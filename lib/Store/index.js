...
export default class Store extends MODULECLASS {
    ...
    _extractPaths(data) {
        return data.childs.map(d => { // d is a file or directory
            ...
            d.filePath ? d.thumbnailPath = `${this.thumbnailPath}/${d.thumbnailPathsCrumped.join('/')}/${d.hash}` : null;
        });
    }
     ...
     *
     * @param hash      a string
     * @param count     how deep, the directory deep
     * @param digits    how much digits have one directory count
     * @returns {*[]}   the chunks
     * @private
     */
